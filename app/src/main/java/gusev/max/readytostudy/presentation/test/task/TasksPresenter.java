...
public class TasksPresenter extends MviBasePresenter<TasksView, BaseViewState> {
    ...
    protected void bindIntents() {
        Observable<BaseViewState> getData = intent(TasksView::getData).flatMap(testModel -> interactor
            .getData(testModel)
            .subscribeOn(Schedulers.io()));

        Observable<BaseViewState> addAnswer = intent(TasksView::onAnswerClicked).flatMap(task -> interactor
            .addAnswer(task)
            .subscribeOn(Schedulers.io()));

        Observable<BaseViewState> getNextTask = intent(TasksView::getNextTask).flatMap(tasks -> interactor.getNextTask(tasks).subscribeOn(Schedulers.io()));
